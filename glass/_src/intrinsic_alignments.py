...
def Epsilon2_NLA(cosmo, z, sxy, A1):
    gz = ccl.growth_factor(cosmo, 1./(1+z))
    Fact = -A1 * 5e-14 * rho_crit * cosmo['Omega_m'] / gz
    e2_NLA = 2 * Fact * sxy
    return e2_NLA
...
def Epsilon2_TT(cosmo, z, sxy, sxx, syy, A2):
    gz = ccl.growth_factor(cosmo, 1./(1+z)) # FIXME: use jax growth factor
    Fact = -5 * A2 * 5e-14 * rho_crit * cosmo['Omega_m'] / gz**2
...
def TATT(cosmo, z, A1, bTA, A2, tidal_field, delta):
    ...
    epsilon_IA_TATT = epsilon_NLA * (1. + bTA * delta) + epsilon_TATT #FIXME: check sign bTA
...
def get_IA(z, density_planes, A1=0.18, bTA=0.8, A2=0.1, model='NLA'):
    ...
    h=0.7, sigma8 = 0.801, n_s= 0.963,w0=-1.00,wa=0.0, Omega_k=0.0) #FIXME: change to jax cosmo
