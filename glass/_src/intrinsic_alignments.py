...
def NLA(cosmo, z, A1, tidal_tensor_map):
    
    s11 = tidal_tensor_map[:,0]
    s22 = tidal_tensor_map[:,1]
    s12 = tidal_tensor_map[:,2]

    e1_NLA = Epsilon1_NLA(cosmo, z, s11, s22)
    e2_NLA = Epsilon2_NLA(cosmo, z, s12)

    epsilon_NLA = jnp.empty(e1_NLA.shape, dtype=complex)
    epsilon_NLA.real = A1 * e1_NLA
    epsilon_NLA.imag = A1 * e2_NLA

    return epsilon_NLA

def TATT(cosmo, z, A1, bTA, A2, tidal_tensor_map, delta):
    ...
    e1_TT = Epsilon1_TT(cosmo, z, s11, s22)
    e2_TT = Epsilon2_TT(cosmo, z, s12, s11, s22)
